...
#[no_mangle]
pub extern "C" fn sc_execute(json_ptr: *const c_char) -> *mut c_char {
    if json_ptr.is_null() {
        return std::ptr::null_mut();
    }
    let c_str = unsafe { CStr::from_ptr(json_ptr) };
    let input = c_str.to_str().unwrap_or("");
    let mut out = sim::process_request(input);
...
#[no_mangle]
pub extern "C" fn sc_free(ptr: *mut c_char) {
    if ptr.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "system" fn Java_sim_NativeSim_scExecuteJNI(
    mut env: JNIEnv,
    _class: JClass,
    request: JString,
) -> jstring {
